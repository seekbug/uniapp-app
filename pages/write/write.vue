        ...
        <view class="artList">
            <block v-for="(item, index) in artList" :key="index">
               ...
               <view class="item" v-if="item.type == 'text'">
                    <textarea :value="item.content" placeholder="请输入文本" />
...
export default {
    ...
    onLoad : function() {
		...
		//检查是否登录 如果未登录就跳到login.vue进行登录成功后跳到../write/write
	...
	methods:{
		...
		uploadImg : function(){
			...
			var uploader = uni.uploadFile({
				...
				success: (uploadFileRes) => {
					...
					console.log('_self.artList',_self.artList); 
					_self.uploadIndex ++;
					// 递归上传
		...
		cateChange(e){
			...
			_self.currentCateIndex = sedIndex;
			// 获取选择的分类名称
			if(sedIndex < 1){return ;}
			var cateName = _self.caties[sedIndex];
			for(var k in  _self.catiesFromApi){
				if(cateName ==  _self.catiesFromApi[k]){
					   _self.sedCateIndex = k;
					  break;
				}
			}
			_self.currentCateIndex = sedIndex;
